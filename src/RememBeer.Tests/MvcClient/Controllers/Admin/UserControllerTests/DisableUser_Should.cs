...
{
    [TestFixture]
    public class DisableUser_Should : UsersControllerTestBase
    {
        [TestCase("")]
        [TestCase(null)]
        [TestCase("user001sdkjasdjklasd789@#&*(23789123789sd")]
        public async Task Call_UserServiceDisableUserAsyncOnceWithCorrectParams(string userId)
        {
            // Arrange
            ...
            // Act
            await sut.DisableUser(userId);

            // Assert
            userService.Verify(s => s.DisableUserAsync(userId), Times.Once);
        }
        ...
        public async Task Return_CorrectRedirectToActionResult()
        {
            ...
            // Act
            var result = await sut.DisableUser("sa", expectedPage, expectedPageSize, expectedSearch) as RedirectToRouteResult;
