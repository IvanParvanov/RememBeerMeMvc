...
{
    [TestFixture]
    public class MakeAdmin_Should : UsersControllerTestBase
    {
        [TestCase("")]
        [TestCase(null)]
        [TestCase("user001sdkjasdjklasd789@#&*(23789123789sd")]
        public async Task Call_UserServiceMakeAdminAsyncOnceWithCorrectParams(string userId)
        {
            // Arrange
            ...
            // Act
            await sut.MakeAdmin(userId);

            // Assert
            userService.Verify(s => s.MakeAdminAsync(userId), Times.Once);
        }
        ...
        public async Task Return_CorrectRedirectToActionResult()
        {
            ...
            var sut = this.Kernel.Get<UsersController>();
            // Act
            var result = await sut.MakeAdmin("sa", expectedPage, expectedPageSize, expectedSearch) as RedirectToRouteResult;
